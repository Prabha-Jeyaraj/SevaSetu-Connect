    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          {/* Brand Logo & Name - Clickable Home Navigation */}
          <button 
            type="button"
            className="flex items-center gap-2.5 cursor-pointer group text-left p-1 -ml-1 rounded-2xl hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-brand-500/20 transition"
            onClick={() => {
              setCurrentTab('landing');
              window.scrollTo({ top: 0, behavior: 'smooth' });
            }}
            title="Return to SevaSetu Connect Home"
            aria-label="Return to SevaSetu Connect Home"
          >
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-600 to-coop-blue flex items-center justify-center text-white shadow-md group-hover:scale-105 group-hover:shadow-lg transition">
              <Layers className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <span className="font-extrabold text-lg tracking-tight text-slate-900 group-hover:text-brand-700 transition">
                  SevaSetu
                </span>
                <span className="text-xs font-bold text-brand-700 bg-brand-50 px-1.5 py-0.5 rounded border border-brand-200 group-hover:border-brand-300 transition">
                  Connect
                </span>
              </div>
              ...
              <div className="relative">
                ...
                {userDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-56 rounded-2xl bg-white shadow-xl border border-slate-100 py-2 z-50 animate-fadeIn">
                    ...
                    <div className="py-1">
                      <button
                        onClick={() => { setCurrentTab('landing'); setUserDropdownOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
